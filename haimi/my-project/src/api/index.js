...
for (let i = 0; i < 100; i++) {
  let type = Random.integer(1, 2)
    getShowsByCityCode.data.push({
      ...
      yue: '【'+Random.integer(1, 12)+'月发货】',
      director: Random.cname(),
      price:'$'+Random.float(1,100,2,2),
      image: Random.dataImage('156x156'),
...
for (let i = 0; i < 100; i++) {
  fenleiList.data.push({
    name:Random.csentence(),
    image: Random.dataImage('320x119'),
    price:'$'+Random.float(1,100,2,2),
    lists:[Random.cname(),Random.cname(),Random.cname(),Random.cname(),Random.cname(),Random.cname(),Random.cname(),Random.cname()]
  })
}
